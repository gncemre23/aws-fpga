...
uint32_t golden_i[40] = {0};
uint32_t circ_buffer[5] = {0};
...
void report_summary_log(bool force)
{
   ...
   double shrate = share_time == 0. ? 0. : exp32 * last_targetdiff * (double)(accepts) / share_time;
   double sess_hrate = uptime.tv_sec == 0. ? 0. : exp32 * norm_diff_sum / (double)uptime.tv_sec;
   ...
   applog2(LOG_INFO, "Share rate        %.2f/min     %.2f/min",
           submit_rate, (double)submitted_share_count * 60. / ((double)uptime.tv_sec + (double)uptime.tv_usec / 1e6));
   ...
   if (accepted_share_count < submitted_share_count)
   {
      double lost_ghrate = uptime.tv_sec == 0 ? 0.
                                              : exp32 * last_targetdiff * (double)(submitted_share_count - accepted_share_count) / (double)uptime.tv_sec;
      double lost_shrate = share_time == 0. ? 0.
                                            : exp32 * last_targetdiff * (double)(submits - accepts) / share_time;
      char lshr_units[4] = {0};
      char lghr_units[4] = {0};
...
*/
   ...
   if (likely(result))
   {
      ...
      if unlikely ((my_stats.net_diff > 0.) && (my_stats.share_diff >= net_diff))
      {
         solved = true;
         solved_block_count++;
   ...
   else
   {
      sprintf(ares, "A%d", accepted_share_count);
      sprintf(bres, "B%d", solved_block_count);
      stale = work ? work->data[algo_gate.ntime_index] != g_work.data[algo_gate.ntime_index] : false;
      if (reason)
         stale = stale || strstr(reason, "Invalid job id");
      if (stale)
      {
         stale_share_count++;
...
start:
   ...
   if (rc)
   {
      ...
      if (work->height > last_block_height)
      {
         ...
         applog(LOG_BLUE, "numcpu: %d, New Work: Block %d, Net Diff %.5g, Ntime %08x",
                num_cpus, work->height, net_diff,
                work->data[algo_gate.ntime_index]);
   } // rc
...
static void stratum_gen_work(struct stratum_ctx *sctx, struct work *g_work)
{
   ...
   bool new_job = *get_stratum_job_ntime() != g_work->data[algo_gate.ntime_index];
   ...
   if (stratum_diff != sctx->job.diff)
      ...
      applog(LOG_BLUE, "numcpus %d, New Work: Block %d, Net diff %.5g, Job %s",
             num_cpus, sctx->block_height, net_diff, g_work->job_id);
   else if (!opt_quiet)
   {
   ...
   while (1)
   {
      ...
      if (likely(algo_gate.do_this_thread(thr_id)))
      {
         ...
         else
         {
            ...
            if (((time(NULL) - g_work_time) >= (have_longpoll ? LP_SCANTIME : opt_scantime)) || (*nonceptr >= end_nonce))
            {
      ...
      {
         int wait_time = curr_temp >= 80 ? 30 : curr_temp >= 70 ? 60
                                                                : 120;
...
static void *stratum_thread(void *userdata)
{
   ...
   while (1)
   {
      ...
      report_summary_log((stratum_diff != stratum.job.diff) && (stratum_diff != 0.));

      if (stratum.job.job_id && (!g_work_time || (*get_stratum_job_ntime() != g_work.data[algo_gate.ntime_index])))
         stratum_gen_work(&stratum, &g_work);
...
void parse_arg(int key, char *arg)
{
   ...
   case 'o': // url
   {
      ...
      if (ap != arg)
      {
         if (strncasecmp(arg, "http://", 7) && strncasecmp(arg, "https://", 8) && strncasecmp(arg, "stratum+tcp://", 14) && strncasecmp(arg, "stratum+tcps://", 15))
         {
...
*/
   ...
   if (!opt_benchmark)
      if (strncasecmp(rpc_url, "https:", 6) && strncasecmp(rpc_url, "stratum+tcps://", 15))
